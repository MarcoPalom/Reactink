...
const SublimateAreaList: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log('Fetched data:', { orders: ordersData, materials: materialsData, quotations: quotationsData, quotationsDesigns: quotationsDesignData })
    } catch (error) {
  ...
  const checkProductStatus = async (products: (FormDataShirtView | FormDataShortView)[]) => {
    const validatedProductsData = await Promise.all(
      products.map(async (product) => {
        ...
        if ( status.cuttingArea && status.printingArea && !status.sublimationArea ) {
          return product
        }
        return null
      })
    ...
    try {
      ...
      const productsWithStatus = await Promise.all(
        quotationProducts.map(async (product) => {
          const status = await fetchProductStatus(product.id, isShortProduct(product) ? 'short' : 'shirt')
          return { ...product, isSublimateAreaComplete: status.SublimateArea }
        })
      )
      console.log('Products with status:', productsWithStatus)
      setFilteredQuotationProducts(productsWithStatus.filter(product => !product.isSublimateAreaComplete))
